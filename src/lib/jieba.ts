...
/** [word, frequency] pair */
export type JiebaDictEntry = [string, number];

interface JiebaCache {
  trie: TrieNode;
  min_freq: number;
  dict_file: string[];
...
export function max_of_array(arr: number[]): number {
 ...
 */
export function loadDictFromText(text: string): JiebaDictEntry[] {
  const lines = text.split('\n');
  const dictionary: JiebaDictEntry[] = [];
 ...
 */
export function buildFromDict(dictionary: JiebaDictEntry[]): [TrieNode, Record<string, number>, number] {
  const trie: TrieNode = {};
...
export class Jieba {
  options: { dictPath?: string; [key: string]: any };
  _cache_: JiebaCache;
  dictionary: JiebaDictEntry[];
  initialized: boolean;
  _waiting: Promise<this> | null;

  constructor(options: { dictPath?: string; [key: string]: any } = {}) {
  ...
  static get DEFAULT_DICT_PATH(): string {
    return 'lib/jieba/dict.txt.big';
  }

  useDict(dict: string | JiebaDictEntry[] | ((this: Jieba, dictionary: JiebaDictEntry[], jieba: Jieba) => any)): this {
  ...
  async cut(sentence: string, cb?: (ret: string[]) => void): Promise<string[]> {
    await this.init();
  ...
  cutSync(sentence: string, cb?: (ret: string[]) => void): string[] {
    this.initSync();
    let ret = this._cut(sentence);
    if (cb) {
      cb(ret);
  ...
  _cut(sentence: string): string[] {
    ...
    for (let b in blocks) {
      let blk: string = (blocks as any)[b];
      if (blk.length == 0) {
        continue;
      }
      ...
      if (blk.match(re_han)) {
        ...
        for (let w in cutted) {
          let word: string = (cutted as any)[w];
  ...
  _get_cut_block(HMM: boolean): (blk: string) => string[] {
    // Bind the method to preserve 'this' context
    return (HMM ? (this as any).__cut_DAG.bind(this) : this.__cut_DAG_NO_HMM.bind(this));
  }

  async _setup(): Promise<this> {
    if (this.initialized) {
      return this;
    }
  ...
  _get_DAG(sentence: string): Record<number, number[]> {
  ...
  _calc(sentence: string, DAG: Record<number, number[]>, idx: number, route: Record<number, [number, number | '']>): void {
  ...
  __cut_DAG_NO_HMM(sentence: string): string[] {
    ...
    let route: Record<number, [number, number | '']> = {};
    let yieldValues: string[] = [];
