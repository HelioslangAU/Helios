class PageProcessor {
  ...
  extractPotentialWords(text, adapter) {
    ...
    // For character-based languages, extract candidate sequences up to the
    // adapter's configured max length so longer Japanese words get preloaded too.
    if (adapter.getScanResolution && adapter.getScanResolution() === 'char') {
      const maxWordLength = Math.max(1, adapter.getConfig?.().maxWordLength || 5);
      for (let i = 0; i < text.length; i++) {
        if (adapter.isTargetCharacter && adapter.isTargetCharacter(text[i])) {
          for (let len = 1; len <= Math.min(maxWordLength, text.length - i); len++) {
  ...
  async tryGetCharacterAtPoint(x, y) {
    ...
    if (parentSpan && parentSpan.hasAttribute('data-word')) {
      ...
      if (wordIndex !== -1 && offset >= wordIndex && offset < wordIndex + wordText.length) {
        const dictionaryForm = parentSpan.getAttribute('data-dictionary-form') || null;
        return {
          word: word,
          textNode: textNode,
          start: wordIndex,
    ...
    for (const wordData of words) {
      if (offset >= wordData.start && offset < wordData.end) {
        return {
          ...
          dictionaryForm: wordData.dictionaryForm || null
        };
      }
    }
