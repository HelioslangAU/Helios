...
export class ChineseLanguageAdapter extends BaseLanguageAdapter {
  jieba: Jieba | null;
  jiebaInitialized: boolean;
  jiebaCache: Map<string, string[]>;
  jiebaCacheMaxSize: number;
   ...
   */
  override async extractWords(text: string, dictionary: any): Promise<ExtractedWord[]> {
    ...
    if (this.jieba && this.jiebaInitialized) {
      try {
        ...
        const chunks: Array<{ text: string; start: number; isTargetLang: boolean }> = [];
        let i = 0;
   ...
   * @param cedictText - Raw CEDICT text
   * @returns Unified dictionary format
   */
  override parseDictionary(cedictText: string): Record<string, any[]> {
   ...
   * @returns Pinyin pronunciation or null
   */
   ...
   * @returns Regex for splitting Chinese sentences
   */
   ...
   * @param syllable - Pinyin syllable with tone number
   * @returns Pinyin with tone marks
   */
  decodePinyinSyllable(syllable: string): string {
   ...
   * @returns Path to CEDICT file
   */
  override getDictionaryPath(): string | undefined {
    return;
  }

  override getDictionaryDownloadUrl(): string {
   ...
   * @param level - Proficiency level (e.g., 'HSK1', 'HSK2')
