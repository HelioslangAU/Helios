...
/** Language configuration metadata shared by all adapters */
export type LanguageConfig = {
  ...
  characterRanges: Array<{ start: number; end: number }>; // Array of {start, end} Unicode ranges
  wordBoundaryRegex: RegExp;
  sentenceBoundaryRegex: RegExp;
  numOfDicts?: number;
};

/** A word (or non-word chunk) extracted from text with its position */
...
/** Result of detecting a variant pattern in a definition */
export interface VariantPatternInfo {
  pattern: string;
  baseWords: string[];
  fullMatch: string;
}
...
export class BaseLanguageAdapter {
  static requiredConfig: Record<string, string> = {
    code: 'string',         // ISO 639-1 language code
    name: 'string',         // Language name
    displayName: 'string',  // Display name for UI
    maxWordLength: 'number',
   ...
   * @returns True if character belongs to this language
   */
   ...
   */
  extractWords(text: string, dictionary: any): ExtractedWord[] | Promise<ExtractedWord[]> {
   ...
   * @returns Unified dictionary format: {word: [{definition, pronunciation, ...}]}
   */
   ...
   * @param entries - Dictionary entries for the word
   * @returns Pronunciation string or null if not found
   */
   ...
   * @returns Regex for splitting sentences
   */
  getSentenceBoundary(): RegExp {
    throw new Error('getSentenceBoundary() must be implemented by language adapter');
  }

  /**
   * Get language configuration metadata
   * @returns Language config: {code, name, maxWordLength, hasSpaces, etc.}
   */
  /**
   ...
   * @returns Scan resolution
   */
  getScanResolution(): string {
    return this.getConfig().scanResolution;
  }

  /**
   * Get case sensitivity setting
   * @returns Whether the language is case sensitive
   */
   ...
   */
  getDictionaryPath(): string | undefined {
   ...
   */
  isValidWord(word: string, dictionary: any): boolean {
   ...
   * @param definition - Definition text to check
   * @returns Object with {pattern, baseWords} or null if not a variant
   */
  detectVariantPattern(definition: string): VariantPatternInfo | null {
   ...
   * @param baseWordsText - Text containing base word(s)
   * @returns Array of base words
   */
  extractBaseWords(baseWordsText: string): string[] {
   ...
   * @returns Array of trimmed, non-empty definition parts
   */
  getDefinitionParts(definition: string): string[] {
   ...
   * @param definition - Original variant definition
   * @param dictionary - Dictionary object to look up base variant
   * @param getDefinitionAsync - Async function to get definition if not in dictionary
   * @param allEntriesForWord - Optional. All dictionary entries for the current word.
   ...
   * @returns Enhanced definition with base variant's definition appended
   */
  async enhanceVariantDefinition(
    definition: string,
    dictionary: any,
    getDefinitionAsync: ((word: string) => Promise<any>) | null = null,
    allEntriesForWord: any[] | null = null
  ): Promise<string> {
   ...
   * @param entries - Dictionary entries for base variant
   * @returns All definitions joined together or null
   */
  extractBaseDefinition(entries: any[]): string | null {
    ...
    for (const entry of entriesToUse) {
      if (entry && entry.definition) {
        ...
        const defs = entry.definition.split(';').map((d: string) => d.trim()).filter(Boolean);
   ...
   * @param level - Proficiency level (e.g., 'A1', 'HSK1')
   * @returns Path to vocabulary file or null if not available
   */
  getOnboardingVocabPath(level: string): string | null {
   ...
   * @param nativeLanguageCode - Native language code (optional, defaults to 'en')
   * @returns URL to download the dictionary zip file
   */
  getDictionaryDownloadUrl(nativeLanguageCode: string | null = 'en'): string {
    const config = this.getConfig();
