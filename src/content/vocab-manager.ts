export class VocabManager {
  ...
  knownWordsByLanguage: Record<string, Set<string>>;
  ignoredWordsByLanguage: Record<string, Set<string>>;
  learningWordsByLanguage: Record<string, Set<string>>;
  ...
  getCurrentLanguageIgnoredWords(): Set<string> {
    // Create set for current language if it doesn't exist (lazy initialization)
  ...
  getCurrentLanguageLearningWords(): Set<string> {
    // Create set for current language if it doesn't exist (lazy initialization)
  ...
  async loadKnownWords(): Promise<void> {
    try {
  ...
  async saveKnownWords(): Promise<void> {
    try {
      // Convert Sets to Arrays for storage
      const knownWordsObj: Record<string, string[]> = {};
      const ignoredWordsObj: Record<string, string[]> = {};
      const learningWordsObj: Record<string, string[]> = {};
  ...
  async clearKnownWords(): Promise<void> {
  ...
  async importKnownWordsFromFile(file: File): Promise<boolean> {
    try {
  ...
  async importKnownWordsFromJson(jsonString: string): Promise<boolean> {
    try {
  ...
  async markWordAsKnown(word: string): Promise<void> {
    ...
    if (wordEntries && Array.isArray(wordEntries) && wordEntries.length > 0) {
      ...
      const nonLemmaEntry = wordEntries.find((entry: any) =>
  ...
  async markWordAsIgnored(word: string): Promise<void> {
    const t0 = performance && typeof performance.now === 'function' ? performance.now() : Date.now();
    const normalizedWord = this.normalizeWord(word);
    if (!normalizedWord) return;
  ...
  async markWordAsUnignored(word: string): Promise<void> {
    const t0 = performance && typeof performance.now === 'function' ? performance.now() : Date.now();
    const normalizedWord = this.normalizeWord(word);
    if (!normalizedWord) return;
    this.getCurrentLanguageIgnoredWords().delete(normalizedWord);
  ...
  async markWordAsLearning(word: string): Promise<void> {
    const t0 = performance && typeof performance.now === 'function' ? performance.now() : Date.now();
    const normalizedWord = this.normalizeWord(word);
    if (!normalizedWord) return;
  ...
  async markWordAsUnlearning(word: string): Promise<void> {
    const t0 = performance && typeof performance.now === 'function' ? performance.now() : Date.now();
    const normalizedWord = this.normalizeWord(word);
    if (!normalizedWord) return;
    this.getCurrentLanguageLearningWords().delete(normalizedWord);
  ...
  isWordLearning(word: string): boolean {
    const normalizedWord = this.normalizeWord(word);
    if (!normalizedWord) return false;
  ...
  async markMultipleWordsAsKnown(words: string[]): Promise<{ newWordsCount: number; processedWordsCount: number }> {
  ...
  async markMultipleWordsAsUnknown(words: string[]): Promise<void> {
    const currentSet = this.getCurrentLanguageKnownWords();
    words.forEach(word => {
      const normalizedWord = this.normalizeWord(word);
      if (normalizedWord) {
  ...
  async markMultipleWordsAsLearning(words: string[]): Promise<{ newWordsCount: number; processedWordsCount: number }> {
  ...
  notifySidebarUpdate(changedWords: string | string[] | null = null, isKnownOrIgnored: boolean = true): void {
    const t0 = performance && typeof performance.now === 'function' ? performance.now() : Date.now();
  ...
  getAllKnownWordsAllLanguages(): Record<string, string[]> {
   ...
   * @param word - Word that was looked up
   * @param definition - Definition data
   */
  trackWordLookup(word: string, definition: any = null): void {
    if (!word) return;
