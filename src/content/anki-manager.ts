...
export class AnkiManager {
  ...
  dictionaryManager: DictionaryManager | DictionaryManagerProxy | null;
  status: AnkiStatus;
  hasSyncedOnConnection: boolean;
  ...
  initialize(dictionaryManager: DictionaryManager | DictionaryManagerProxy): void {
  ...
  // Get current status
  async getStatus(): Promise<AnkiStatus> {
    await this.checkAnkiConnect();
    return this.status;
  }

  // Get quick status for popup
  async getQuickStatus(): Promise<{ available: boolean; message: string }> {
    try {
  ...
  formatDefinitionForAnki(definition: string): string {
  ...
  async captureMediaForCard(wordData: AnkiWordData): Promise<void> {
    try {
  ...
  async checkIfNeedsMedia(): Promise<boolean> {
    try {
  ...
  async captureAudioForSentence(sentence: string | undefined): Promise<string | null> {
    try {
  ...
  getSubtitleTimingForSentence(sentence: string): SubtitleTiming | null {
    try {
  ...
  extractSentenceContext(character: string): string {
    try {
  ...
  getContextFromElement(element: Element, character: string): string {
  ...
  async createCard(data: string | AnkiWordData, options: Record<string, any> = {}): Promise<{ success: boolean; noteId?: any; message?: string; error?: string }> {
    try {
  ...
  async createCardFromPopup(wordData: string | AnkiWordData, button: HTMLButtonElement | null, frequencyManager: FrequencyManager | null = null): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  updateButtonState(button: HTMLButtonElement | null, state: string, errorMessage: string = ""): void {
    if (!button) return;
  ...
  categorizeError(errorMessage: string): { icon: string; message: string; class: string } {
  ...
  async getNoteTypeFields(noteType: string): Promise<string[]> {
    try {
  ...
  // Test connection
  async testConnection(): Promise<{ success: boolean; message: string; version?: any }> {
    try {
      const response = await this.sendMessage("ANKI_TEST_CONNECTION");
      return {
  ...
  async syncLearningWordsFromAnki(): Promise<{ synced: number; promoted: number; error?: string }> {
    try {
      ...
      const expressionToInterval = new Map<string, number>();
  ...
  updatePopupButtonState(character: string): void {
    try {
      ...
      const updateButton = (button: Element) => {
