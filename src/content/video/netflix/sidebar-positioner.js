/**
 * Netflix Sidebar Positioner
 *
 * Handles positioning and sizing of the Helios sidebar relative to the Netflix video player.
 * Keeps the sidebar synchronized with video player dimensions and position.
 *
 * Key approach:
 * - Sidebar is positioned relative to the video player
...
class NetflixSidebarPositioner {
  ...
  _syncPositionImmediate() {
    ...
    if (!videoPlayer || !watchContainer) {
      console.warn('[Helios Netflix Positioner] Video player or watch container not found');
      return;
    }

    // Get dimensions
    const playerRect = videoPlayer.getBoundingClientRect();
    const watchContainerRect = watchContainer.getBoundingClientRect();

    // Calculate position relative to watch container
    const topRelativeToWatch = playerRect.top - watchContainerRect.top;
    ...
    this.sidebar.style.setProperty('top', `${topRelativeToWatch}px`, 'important');
  }
