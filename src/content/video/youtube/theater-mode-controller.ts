 ...
 */
export class TheaterModeController {
  isTheaterMode: boolean;
  theaterChangeCallbacks: Array<(isActive: boolean) => void>;
  enforcementEnabled: boolean;
  enforcementInterval: ReturnType<typeof setInterval> | null;
  theaterObserver: MutationObserver | null;

  // Configuration
  ENFORCEMENT_CHECK_INTERVAL: number;
  MAX_BUTTON_FIND_ATTEMPTS: number;
  BUTTON_FIND_RETRY_DELAY: number;
  THEATER_ACTIVATION_TIMEOUT: number;
   ...
   * @returns True if theater mode is active
   */
  isActive(): boolean {
   ...
   * @private
   * @returns The theater button or null
   */
  async _findTheaterButton(): Promise<HTMLElement | null> {
    let attempts = 0;

    while (attempts < this.MAX_BUTTON_FIND_ATTEMPTS) {
      // Strategy 1: Direct class selector
      let button: HTMLElement | null | undefined = document.querySelector<HTMLElement>('button.ytp-size-button');
      if (button) return button;

      // Strategy 2: Without 'button' prefix
      button = document.querySelector<HTMLElement>('.ytp-size-button');
      if (button) return button;

      // Strategy 3: Search by title/aria-label containing 'theater'
      const allButtons = document.querySelectorAll<HTMLButtonElement>('button.ytp-button');
      ...
      button = document.querySelector<HTMLElement>('button[data-tooltip-target-id*="theater"]');
      if (button) return button;
   ...
   * @param enableEnforcement - Whether to continuously enforce theater mode
   * @returns True if theater mode was enabled successfully
   */
  async enable(enableEnforcement: boolean = true): Promise<boolean> {
   ...
   * @returns True if theater mode was disabled successfully
   */
   ...
   * @private
   * @param expectedState - Expected theater mode state
   */
  _waitForTheaterMode(expectedState: boolean = true): Promise<boolean> {
   ...
   * @param callback - Called with (isTheaterMode: boolean)
   */
  onTheaterModeChange(callback: (isActive: boolean) => void): void {
   ...
   * @private
   */
  _notifyTheaterModeChange(isActive: boolean): void {
   ...
   */
  observeTheaterModeChanges(): void {
