...
export class SubtitleOverlay {
  ...
  _dragUpHandler: (() => void) | null = null;
  _resizeMoveHandler: ((e: MouseEvent) => void) | null = null;
  _resizeUpHandler: (() => void) | null = null;
  ...
  _setupDragging(): void {
    ...
    this.container!.addEventListener('dblclick', (e) => {
      ...
      if ((e.target as HTMLElement).classList.contains('helios-subtitle-word')) {
        return;
      }
   ...
   */
  async _setupResizeShortcuts(): Promise<void> {
   ...
   * @param shortcut - Shortcut config { key, ctrl, shift, alt, meta }
   * @returns True if event matches shortcut
   */
  _matchesShortcut(e: KeyboardEvent, shortcut: any): boolean {
   ...
   */
  _applySubtitleSize(): void {
    // Apply to primary subtitles
    const primarySubtitles = this.container!.querySelectorAll<HTMLElement>('.helios-subtitle-primary');
    primarySubtitles.forEach(el => {
      el.style.fontSize = this.subtitleSize + 'px';
    });

    // Apply to secondary subtitles (slightly smaller)
    const secondarySubtitles = this.container!.querySelectorAll<HTMLElement>('.helios-subtitle-secondary');
   ...
   */
  _setupShiftKeyInterception(): void {
   ...
   */
  _findFullscreenParent(): HTMLElement {
    ...
    let current: HTMLElement | null = this.videoElement.parentElement;
   ...
   * @param subtitles - Secondary subtitle entries
   */
  setSecondarySubtitles(subtitles: SubtitleEntry[]): void {
   ...
   */
  async _savePosition(): Promise<void> {
    try {
      const platform = this._detectPlatform();
      const positionData = {
   ...
   */
  async _saveBackgroundOpacity(): Promise<void> {
    try {
      const platform = this._detectPlatform();
   ...
   * Now platform-specific
   */
  async _saveVisibility(): Promise<void> {
    try {
      const platform = this._detectPlatform();
   ...
   * @param enabled - Whether to pause video on hover
   */
  ...
  /**
   * Display subtitles
   * @param subtitles - Subtitles to display
   */
  show(subtitles: SubtitleEntry[]): void {
  ...
  async _render(): Promise<void> {
    ...
    if ((window as any).pinyinManager && (window as any).pinyinManager.isEnabled()) {
      ...
      textNodes.forEach(node => {
        if ((window as any).pinyinManager.processTextNodeForPinyin) {
          (window as any).pinyinManager.processTextNodeForPinyin(node);
        }
      });
    }
  }
   ...
   */
  _getChineseTextNodes(element: HTMLElement): Node[] {
   ...
   */
  _findMatchingSecondarySubtitle(primarySubtitle: SubtitleEntry): SubtitleEntry | null {
   ...
   */
  _handleSubtitleClick(event: MouseEvent, subtitle: SubtitleEntry): void {
   ...
   */
  getSubtitleBackgroundOpacity(): number {
