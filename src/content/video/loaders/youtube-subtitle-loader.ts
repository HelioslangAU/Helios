...
export class YouTubeSubtitleLoader {
   ...
   * @param trackUrl - YouTube caption track URL
   */
  async loadTrack(trackUrl: string): Promise<SubtitleEntry[]> {
    try {
      // Fetch caption data
   ...
   * @param xml - YouTube caption XML
   */
  _parseYouTubeCaptions(xml: string): SubtitleEntry[] {
    try {
   ...
   */
  _decodeHTMLEntities(text: string): string {
   ...
   * @param preferredLanguage - Language code (e.g., 'en', 'ja', 'zh')
   */
