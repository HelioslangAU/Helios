...
class YouTubeSubtitleLoader {
  ...
  _injectPageScript() {
    ...
    try {
      ...
      script.onload = () => {
        script.remove();
        console.log('[Helios YouTube] Page script injected successfully');
      };
      script.onerror = () => {
  ...
  _setupEventListeners() {
    window.addEventListener('helios-youtube-subtitles-response', (event) => {
      if (this.pendingRequest) {
        ...
        if (success && tracks) {
          console.log('[Helios YouTube] Received tracks from page script:', tracks.length);
          resolve(tracks);
        } else {
  ...
  async loadTrack(trackUrl) {
    try {
      console.log('[Helios YouTube] Fetching track from URL:', trackUrl);
      ...
      console.log('[Helios YouTube] Received XML length:', xml.length);
      console.log('[Helios YouTube] XML preview:', xml.substring(0, 500));

      // Parse XML captions
      const entries = this._parseYouTubeCaptions(xml);
      console.log('[Helios YouTube] Parsed entries:', entries.length);
  ...
  _parseYouTubeCaptions(xml) {
    try {
      ...
      console.log('[Helios YouTube] Parsed XML document');
      ...
      console.log('[Helios YouTube] Found subtitle nodes:', subtitleNodes.length, isSrv3Format ? '(srv3 format)' : '(standard format)');
      ...
      console.log('[Helios YouTube] Created subtitle entries:', entries.length);
      return entries;
    } catch (error) {
  ...
  async autoLoadSubtitles(preferredLanguage = 'en') {
    console.log('[Helios YouTube] Auto-loading subtitles for language:', preferredLanguage);
    ...
    console.log('[Helios YouTube] Available tracks:', tracks.map(t => `${t.language} (${t.languageName})`));
    ...
    if (!selectedTrack) {
      console.log('[Helios YouTube] No manual track for', preferredLanguage, '- trying auto-generated');
      selectedTrack = tracks.find(t => t.language === preferredLanguage);
    }

    // Fallback to partial match (e.g., 'zh' matches 'zh-Hans' or 'zh-Hant')
    if (!selectedTrack) {
      console.log('[Helios YouTube] No exact match - trying partial match');
    ...
    try {
      ...
      if (binding) {
        ...
        console.log(`[Helios YouTube] Loaded ${entries.length} subtitles (${selectedTrack.languageName})`);
        return true;
      } else {
