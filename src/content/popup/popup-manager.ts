...
import { PopupSettingsManager } from '@/content/popup/popup-settings-manager';
import { PopupPronunciationManager } from '@/content/pronunciation';
...
export class PopupManager {
  ...
  isMouseOverPopup: boolean | null;
  isSubtitleWordPopup: boolean;

  // Prevent concurrent popup creation
  popupCreationInProgress: boolean;
  currentPopupRequestId: number | null;

  // Global mouse tracking for better hide detection
  globalMouseMoveHandler: ((e: MouseEvent) => void) | null;
  mouseLeaveHandler: (() => void) | null;
  lastMousePosition: { x: number; y: number };
  mouseCheckInterval: ReturnType<typeof setInterval> | null;
  ...
  constructor({ highlightManager, dictionaryManager, vocabManager, frequencyManager, languageRegistry }: PopupManagerOptions) {
   ...
   * @private
   */
  _startGlobalMouseTracking(): void {
  ...
  _handleMouseOutsideDocument(): void {
  ...
  _isMouseOverRelevantElement(element: Element): { isOverPopup: boolean | null; isOverHighlight: boolean | undefined; isOverSubtitleWord: boolean } {
  ...
  _isScrollbarOrOutside(element: Element, isOverPopup: boolean | null, isOverHighlight: boolean | undefined, isOverSubtitleWord: boolean): boolean {
    return !document.body.contains(element) ||
           element.tagName === 'HTML' ||
           (element === document.body && !isOverPopup && !isOverHighlight && !isOverSubtitleWord);
  }

  _updateHoverStates(elementAtMouse: Element): void {
  ...
  hidePopup(event?: Event): void {
    if (!this.popup || (event?.target && this.popup.contains(event.target as Node))) {
      return;
    }
