import type { DictionaryEntry } from '@/content/popup/components/card-manager';

type FilterType = 'deprioritize' | 'hide' | 'prioritize';

/**
 * DefinitionFilter - Filters and sorts definitions
 * Handles prioritization and hiding of definition entries
 */
export class DefinitionFilter {
  filters: Record<FilterType, RegExp[]>;
  ...
  filterAndSortDefinitions(entries: DictionaryEntry[]): DictionaryEntry[] {
  ...
  getDefinitionPriority(definition: string): number {
  ...
  addFilter(type: string, pattern: string): void {
    if (this.filters[type as FilterType]) {
      this.filters[type as FilterType].push(new RegExp(pattern, "i"));
  ...
  removeFilter(type: string, patternString: string): void {
    if (this.filters[type as FilterType]) {
      const index = this.filters[type as FilterType].findIndex(
        (pattern) => pattern.source === new RegExp(patternString, "i").source
      );
      if (index > -1) {
        this.filters[type as FilterType].splice(index, 1);
  ...
  getFilters(): Record<string, string[]> {
    ...
    (Object.keys(this.filters) as FilterType[]).forEach((type) => {
