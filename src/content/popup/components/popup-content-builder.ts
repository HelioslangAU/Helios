...
import type { DictionaryEntry, PopupDictionaryData, WordCard } from '@/content/popup/components/card-manager';
import type { PopupSettings } from '@/content/popup/popup-settings-manager';
...
export class PopupContentBuilder {
  static getWordLengthClass(word: string): string {
  ...
  static getLanguageClass(languageCode: string): string {
  ...
  static formatFrequency(frequency: number | null | undefined): string | null {
   ...
   * @param grammar - Grammar field content
   * @returns 'masc', 'fem', 'neut', or null
   */
  static extractGender(grammar: string | null | undefined): 'masc' | 'fem' | 'neut' | null {
   ...
   * @returns True if language uses gender
   */
  static languageUsesGender(languageCode: string): boolean {
   ...
   * @returns True if entry is non-lemma
   */
  static isNonLemma(entry: Partial<DictionaryEntry> | null | undefined): boolean {
   ...
   * @param languageCode - Language code
   * @returns HTML string for info boxes
   */
  static createInfoBoxes(
    entry: Partial<DictionaryEntry> | null | undefined,
    ...
    if (entry.partOfSpeech && entry.partOfSpeech.trim()) {
      ...
      const posExpansions: Record<string, string> = {
        'n': 'noun',
        'v': 'verb',
        'adj': 'adjective',
        'adv': 'adverb',
  ...
  static createBasicContent(
    character: string,
    dictionaryData: PopupDictionaryData,
  ...
  static createDefinitionsHtml(entries: DictionaryEntry[], dictionary: any = null): string {
        ...
        if (entry.baseFormDefinitions && Array.isArray(entry.baseFormDefinitions) && entry.baseFormDefinitions.length > 0) {
          ...
          if (baseFormEntries && Array.isArray(baseFormEntries) && baseFormEntries.length > 0) {
            ...
            for (const baseEntry of baseFormEntries) {
              if (baseEntry.definition) {
                const baseDefs = baseEntry.definition.split(";").map((d: string) => d.trim()).filter(Boolean);
                if (baseDefs.length > 0) {
                  const baseBullets = baseDefs.length > 1
                    ? `<ul class="definition-list">${baseDefs.map((d: string) => `<li>${d}</li>`).join("")}</ul>`
                    : `<div class="definition">${baseDefs[0]}</div>`;
                  processedEntries.push(`<div class="definition-block">${baseBullets}</div>`);
                }
              }
  ...
  static createPronunciationButton(character: string, pinyin: string | null | undefined, ttsText: string | null | undefined = null): string {
    return `
      <button
  ...
  static createAnkiButton(): string {
    return `<button class="anki-btn anki-available" title="Add to Anki">A</button>`;
  }

  static createNavigationDots(cards: WordCard[], currentIndex: number): string {
  ...
  static createMarkButton(isKnown: boolean | undefined, isIgnored: boolean | undefined, isLearning: boolean | undefined = false): string {
  ...
  static getMarkButtonClass(isKnown: boolean | undefined, isIgnored: boolean | undefined, isLearning: boolean | undefined = false): string {
    if (isKnown) {
  ...
  static getMarkButtonText(isKnown: boolean | undefined, isIgnored: boolean | undefined, isLearning: boolean | undefined = false): string {
    if (isKnown) {
      return "Known";
    } else if (isLearning) {
      return "Learning";
