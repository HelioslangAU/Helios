...
class LanguageSwitchCoordinator {
  ...
  async switchLanguage(newLanguageCode) {
    ...
    try {
      ...
      // Step 4.5: Reload frequency list for the new language
      if (window.frequencyManager && typeof window.frequencyManager.setLanguage === 'function') {
        window.frequencyManager.setLanguage(newLanguageCode);
        await window.frequencyManager.loadFrequencyList();
        console.log(`✅ Frequency list reloaded for ${newLanguageCode}`);
      }
