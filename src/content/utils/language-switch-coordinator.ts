...
interface LanguageSwitchCoordinatorDeps {
...
export class LanguageSwitchCoordinator {
  languageRegistry: LanguageRegistry;
  dictionaryManager: any;
  pageProcessor: PageProcessor | undefined;
  popup: any;
  vocabManager: VocabManager | undefined;

  isSwitching: boolean;
  currentLanguage: string | null;

  constructor(dependencies: LanguageSwitchCoordinatorDeps) {
   ...
   * @param newLanguageCode - The language code to switch to
   * @returns Success status
   */
  async switchLanguage(newLanguageCode: string): Promise<boolean> {
    if (this.isSwitching) {
   ...
   * Show loading indicator
   */
  _showLoadingIndicator(languageCode: string): void {
   ...
   * Show success notification
   */
  _showSuccessNotification(languageCode: string): void {
    const languages: Record<string, string> = {
      zh: 'Chinese',
      en: 'English',
      es: 'Spanish',
   ...
   */
  _showErrorNotification(error: any): void {
    const notification = document.createElement('div');
    notification.style.cssText = `
      position: fixed;
      top: 20px;
