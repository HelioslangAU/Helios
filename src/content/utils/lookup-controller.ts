...
export class LookupController {
  ...
  constructor({ pageProcessor, highlightManager, popup, activation }: LookupControllerDeps) {
  ...
  onPointerMove = async (event: PointerEvent): Promise<void> => {
    ...
    let checkElement: any = event.target;
  ...
  _isTargetLanguageWord(word: string | null | undefined): boolean {
    if (!word) return false;
    const adapter = (this.pageProcessor as any)?.languageRegistry?.getAdapter?.();
