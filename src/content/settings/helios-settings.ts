...
export class HeliosSettingsManager {
  ...
  storage: HeliosSettingsStorage | null;
  ui: HeliosSettingsUI | null;
  anki: HeliosSettingsAnki | null;
  vocabulary: HeliosSettingsVocabulary | null;
  advanced: HeliosSettingsAdvanced | null;
  ...
  setupEventListeners(): void {
    ...
    navItems.forEach((item) => {
      item.addEventListener("click", (e) => {
        console.log(
          "🔍 DEBUG: Tab clicked:",
          (e.currentTarget as HTMLElement).getAttribute("data-tab")
        );
        this.switchTab(e);
      });
    });
  ...
  async switchTab(event: Event): Promise<void> {
    const targetTab = (event.currentTarget as HTMLElement).getAttribute("data-tab")!;
    ...
    if (targetTab === "anki" && this.anki && !(this.anki as any).ankiConnection) {
  ...
  async loadTabContent(tabName: string): Promise<void> {
    ...
    try {
      ...
      const url = PATHS.url(PATHS.settingsTabHtml(tabName));
