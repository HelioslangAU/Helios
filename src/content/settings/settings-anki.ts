...
export class HeliosSettingsAnki {
  manager: HeliosSettingsManager;
  isConnected: boolean;
  availableDecks: string[];
  availableNoteTypes: string[];
  currentNoteTypeFields: string[];
  ...
  getAvailableDataTypes(): DataTypeOption[] {
  ...
  async initializeAnki(): Promise<void> {
    try {
  ...
  async checkConnection(): Promise<boolean | undefined> {
  ...
  populateNoteTypesDropdown(): void {
  ...
  // Handle deck change
  async onDeckChange(event: Event): Promise<void> {
  ...
  async onNoteTypeChange(event: Event): Promise<void> {
  ...
  async onSettingChange(event: Event): Promise<void> {
    const setting = (event.target as HTMLInputElement).id;
  ...
  async loadNoteTypeFields(noteType: string): Promise<void> {
    try {
  ...
  getFallbackFields(noteType: string): string[] {
  ...
  updateFieldMappingTable(): void {
    ...
    const currentMappings: Record<string, string> = this.manager.settings.ankiFieldMappings || {};
  ...
  generateDataTypeOptions(selectedValue: string = ""): string {
  ...
  getPreviewData(dataType: string): string {
    const language = this.manager.settings.targetLanguage || 'zh';
  ...
  // Setup field mapping event listeners
  setupFieldMappingListeners(): void {
    const mappingSelects = document.querySelectorAll<HTMLSelectElement>(".mapping-select");
  ...
  applyAutoMapping(): void {
    ...
    this.currentNoteTypeFields.forEach((field) => {
      ...
      if (autoMapping) {
        // Update dropdown
        const select = document.querySelector<HTMLSelectElement>(`[data-field="${field}"]`);
        if (select) {
  ...
  async loadCurrentSettings(): Promise<void> {
    try {
      ...
      // Update dropdowns
      const deckSelect = document.getElementById("anki-deck-select") as HTMLSelectElement | null;
      if (deckSelect && settings.deck) {
        deckSelect.value = settings.deck;
        this.manager.settings.ankiDeck = settings.deck;
      }

      const noteTypeSelect = document.getElementById("anki-note-type-select") as HTMLSelectElement | null;
      ...
      // Update toggles
      const checkDuplicates = document.getElementById("anki-check-duplicates") as HTMLInputElement | null;
      if (checkDuplicates) {
        checkDuplicates.checked = settings.checkDuplicates !== false;
        this.manager.settings.ankiCheckDuplicates = checkDuplicates.checked;
      }

      const includeSentence = document.getElementById("anki-include-sentence") as HTMLInputElement | null;
      if (includeSentence) {
      ...
      const importYoungAsLearning = document.getElementById("anki-import-young-as-learning") as HTMLInputElement | null;
      if (importYoungAsLearning) {
      ...
      const autoSyncLearningWords = document.getElementById("anki-auto-sync-learning-words") as HTMLInputElement | null;
      if (autoSyncLearningWords) {
        // Check both settings locations (ankiSettings and manager.settings)
  ...
  async testConnection(): Promise<void> {
    const button = document.getElementById("test-anki-connection") as HTMLButtonElement | null;
    if (!button) return;
  ...
  async getStatus(): Promise<{ connected: boolean; ready: boolean; settings: Record<string, any>; errors: string[] }> {
    const status = {
    ...
    const fieldMappings: Record<string, string> = this.manager.settings.ankiFieldMappings || {};
  ...
  // Create a test card
  async createTestCard(): Promise<void> {
    try {
  ...
  updateImportButtonState(): void {
    const importButton = document.getElementById("anki-import-known-words") as HTMLButtonElement | null;
  ...
  updateSyncButtonState(): void {
    const syncButton = document.getElementById("anki-sync-learning-words") as HTMLButtonElement | null;
  ...
  async importKnownWordsFromAnki(): Promise<void> {
    const importButton = document.getElementById("anki-import-known-words") as HTMLButtonElement | null;
    if (!importButton) return;
    ...
    try {
      ...
      let expressionField: string | null = null;
      ...
      const expressions: any[] = response.expressions || [];
      ...
      const expressionsToImportAsKnown = new Set<string>();
      const expressionsToImportAsLearning = new Set<string>();
  ...
  async syncLearningWordsFromAnki(): Promise<void> {
    const syncButton = document.getElementById("anki-sync-learning-words") as HTMLButtonElement | null;
    if (!syncButton) return;
    ...
    try {
      ...
      ankiManager.initialize((this.manager as any).dictionary?.dictionaryManager);
