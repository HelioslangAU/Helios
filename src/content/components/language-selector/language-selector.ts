...
export interface LanguageSelectorOptions {
  onLanguageSelected?: (languageCode: string, language: LanguageOption) => void;
...
export class LanguageSelector {
  ...
  _attachEventListeners(container: HTMLElement): void {
    if (this.layout === 'grid') {
      const cards = container.querySelectorAll<HTMLElement>('.language-card');
      cards.forEach(card => {
      ...
      const dropdown = container.querySelector<HTMLSelectElement>('.language-dropdown')!;
      dropdown.addEventListener('change', (e) => {
        const target = e.target as HTMLSelectElement;
        if (target.value) {
          this._handleSelection(target.value, container);
        }
      });
    }
  }
   ...
   */
  _handleSelection(languageCode: string, container: HTMLElement): void {
    this.selectedLanguage = languageCode;

    // Update UI
    if (this.layout === 'grid') {
      container.querySelectorAll<HTMLElement>('.language-card').forEach(card => {
