...
export class DictionaryBridge {
   ...
   */
  async loadDictionary(languageCode: string, nativeLanguageCode: string | null = null): Promise<any> {
    try {
   ...
   */
  async getEntries(words: string[]): Promise<Record<string, any>> {
   ...
   * Cache an entry
   */
  cacheEntry(word: string, entries: any): void {
   ...
   */
  async isDictionaryLoaded(): Promise<boolean> {
...
export class DictionaryProxy {
  ...
  cache: Map<string, any>;
  pendingLookups: Map<string, Promise<any>>;

  constructor(bridge: DictionaryBridge) {
    this.bridge = bridge;
    this.cache = new Map();
   ...
   */
  createSyncDictionary(): Record<string, any> {
    const self = this;
 ...
 */
export class DictionaryManagerProxy {
  languageRegistry: LanguageRegistry;
  bridge: DictionaryBridge;
  proxy: DictionaryProxy;
   ...
   */
  async loadDictionary(languageCode: string | null = null, nativeLanguageCode: string | null = null): Promise<void> {
    if (this.isLoading) {
      return;
    }
   ...
   */
  get dictionary(): Record<string, any> {
