...
export class OnboardingPage {
  ...
  nativeLanguageSelector: HTMLElement | null;
  selectedLanguage: LanguageOption | null;
  selectedNativeLanguage: NativeLanguage | null;
  selectedLevel: LevelDefinition | null;
  shouldImportWords: boolean;
  vocabManager: VocabManager | null;

  // Popup system components
  popupSystemInitialized: boolean;
  dictionaryManager: DictionaryManagerProxy | null;
  dictionaryBridge: DictionaryBridge | null; // For early dictionary loading
  highlightManager: HighlightManager | null;
  pageProcessor: PageProcessor | null;
  popupManager: MultiCardPopupManager | null;
  lookupController: LookupController | null;
  activationController: ActivationController | null;
  ...
  dictionaryCheckInterval?: ReturnType<typeof setInterval> | null;

  // Store event handler references for cleanup
  eventHandlers: OnboardingEventHandlers;
  ...
  handleLanguageSelection(code: string, language: LanguageOption): void {
    ...
    // Enable next button
    const nextBtn = document.getElementById('btn-lang-next') as HTMLButtonElement | null;
    if (nextBtn) {
      nextBtn.disabled = false;
    }
  }

  goToNativeLanguageSelection(): void {
  ...
  initializeNativeLanguageSelector(): void {
    ...
    const nativeLanguages: NativeLanguage[] = [
    ...
    nativeLanguages.forEach(lang => {
      ...
      langCard.addEventListener('click', () => {
        ...
        // Enable next button
        const nextBtn = document.getElementById('btn-native-lang-next') as HTMLButtonElement | null;
        if (nextBtn) {
          nextBtn.disabled = false;
        }
      });
  ...
  async startDictionaryLoading(languageCode: string): Promise<void> {
    try {
      ...
      const nativeLangCode = this.selectedNativeLanguage ? this.selectedNativeLanguage.code : null;
      this.dictionaryBridge.loadDictionary(languageCode, nativeLangCode).then((result: any) => {
        if (result.success) {
  ...
  resetDictionaryLoadingBar(): void {
    const container = document.getElementById('dictionary-loading-container');
  ...
  completeDictionaryLoadingBar(): void {
    const container = document.getElementById('dictionary-loading-container');
    const loadingText = container?.querySelector('.loading-text');
  ...
  startProgressTracking(languageCode: string): void {
    ...
    const checkDictionaryLoaded = async (): Promise<boolean> => {
      try {
        if (this.dictionaryBridge) {
  ...
  async goToTutorial(): Promise<void> {
    if (!this.selectedLanguage) {
      alert('Please select a language before continuing');
      return;
    }
  ...
  async initializeLevelSelector(): Promise<void> {
    ...
    const levels: LevelDefinition[] = adapter.getLevelDefinitions();
  ...
  async getLanguageAdapter(languageCode: string): Promise<BaseLanguageAdapter | null> {
    ...
    try {
      ...
      const adapters: Record<string, new () => BaseLanguageAdapter> = {
  ...
  async handleLevelSelection(): Promise<void> {
    ...
    if (this.shouldImportWords) {
      try {
        const nextBtn = document.getElementById('btn-level-next') as HTMLButtonElement | null;
        if (nextBtn) {
  ...
  async skipLevelSelection(): Promise<void> {
  ...
  updateRecommendedContent(): void {
    ...
    const recommendedVideos: Record<string, string> = {
  ...
  async checkDictionaryLoaded(): Promise<boolean> {
    try {
      if (this.dictionaryBridge) {
  ...
  async importWordsForLevel(languageCode: string, level: LevelDefinition): Promise<any> {
  ...
  async handleBulkImport(): Promise<void> {
    const textarea = document.getElementById('bulk-import-textarea') as HTMLTextAreaElement;
  ...
  updateProgressIndicator(stepName: string): void {
  ...
  getSampleSentence(languageCode: string): string {
  ...
  async initializePopupSystem(): Promise<void> {
    if (!this.selectedLanguage) {
    ...
    try {
      ...
      let dictionaryLoadPromise: Promise<any>;
      ...
      this.highlightManager.highlightLookupText = (node: Text, start: number, end: number) => {
      ...
      this.eventHandlers.keyup = (e: KeyboardEvent) => {
        if (this.activationController && this.lookupController) {
      ...
      this.eventHandlers.click = (e: MouseEvent) => {
        if (this.lookupController) {
