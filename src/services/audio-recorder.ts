...
export class HeliosAudioRecorder {
    isRecording: boolean;
     ...
     * @param videoElement - The video element to record from
     * @param seekToTimeSec - Time in seconds to seek to (already includes padding)
     * @param durationMs - Duration in milliseconds to record
     * @returns Data URL of recorded audio (WebM/Opus format)
     */
    async recordFromVideo(videoElement: HTMLVideoElement, seekToTimeSec: number, durationMs: number): Promise<string | null> {
        if (!videoElement) {
        ...
        try {
            ...
            const stream: MediaStream = (videoElement as any).captureStream ?
     ...
     * @private
     */
    _checkCaptureStreamSupport(videoElement: HTMLVideoElement): boolean {
     ...
     * @private
     */
    _waitForSeek(videoElement: HTMLVideoElement): Promise<void> {
     ...
     * @private
     */
    _recordStream(stream: MediaStream, durationMs: number): Promise<string | null> {
        return new Promise((resolve) => {
            try {
                this.audioChunks = [];
...
if (typeof window !== 'undefined') {
    (window as any).HeliosAudioRecorder = new HeliosAudioRecorder();
