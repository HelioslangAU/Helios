...
type InvokeAnkiFunc = (action: string, params: Record<string, any>) => Promise<any>;
...
export class MediaStorage {
     ...
     * @param dataUrl - Base64 data URL of the media file
     * @param type - Type of media: 'screenshot' or 'audio'
     * @param invokeAnkiFunc - Reference to AnkiConnect invoke function
     * @returns Anki field value (HTML for image, [sound:] for audio) or null on error
     */
    static async storeMediaFile(dataUrl: string | null | undefined, type: string, invokeAnkiFunc: InvokeAnkiFunc): Promise<string | null> {
        if (!dataUrl) {
     ...
     */
    static extractBase64(dataUrl: string): string | null {
     ...
     * @param type - 'screenshot' or 'audio'
     */
     ...
     * @param type - 'screenshot' or 'audio'
     */
    static formatAsAnkiField(fileName: string, type: string): string {
        if (type === 'screenshot') {
            // Image HTML tag
     ...
     * @param invokeAnkiFunc - Reference to AnkiConnect invoke function
     */
    static async deleteMediaFile(fileName: string, invokeAnkiFunc: InvokeAnkiFunc): Promise<boolean> {
        try {
     ...
     * @param invokeAnkiFunc - Reference to AnkiConnect invoke function
     */
    static async mediaFileExists(fileName: string, invokeAnkiFunc: InvokeAnkiFunc): Promise<boolean> {
        try {
     ...
     */
    static validateDataUrl(dataUrl: string | null | undefined, type: string): DataUrlValidationResult {
        const result: DataUrlValidationResult = {
            valid: true,
            error: null,
            size: 0
        };
