...
class BackgroundService {
  ...
  pendingDictionaryRequests!: Map<number | string, PendingDictionaryRequest>;
  ...
  async handleAnkiTestConnection(sendResponse: SendResponse): Promise<void> {
    try {
  ...
  async handleAnkiCreateCard(wordData: any, options: any = {}, sendResponse?: SendResponse): Promise<void> {
    try {
  ...
  async handleAnkiGetDecks(sendResponse: SendResponse): Promise<void> {
    try {
      const decks: string[] = await this.invokeAnki("deckNames");
      sendResponse({
        success: true,
        decks: decks.sort(),
      });
  ...
  async handleAnkiGetNoteTypes(sendResponse: SendResponse): Promise<void> {
    try {
      const noteTypes: string[] = await this.invokeAnki("modelNames");
      sendResponse({
        success: true,
  ...
  async handleAnkiGetNoteTypeFields(noteType: string, sendResponse: SendResponse): Promise<void> {
    try {
  ...
  async handleAnkiGetDeckNotes(deck: string, noteType: string, expressionField: string, sendResponse: SendResponse): Promise<void> {
    try {
      if (!deck || !noteType) {
      ...
      const noteIds: number[] = await this.invokeAnki("findNotes", { query });
      ...
      const allCardIds: any[] = [];
      const noteIdToCardIds = new Map<any, any[]>();

      // First pass: Get card IDs from notes (in batches)
      for (let i = 0; i < noteIds.length; i += BATCH_SIZE) {
        const batchNoteIds = noteIds.slice(i, i + BATCH_SIZE);
        const batchNotes: any[] = await this.invokeAnki("notesInfo", { notes: batchNoteIds });
      ...
      const cardIdToInterval = new Map<any, number>();
      if (allCardIds.length > 0) {
        // Process cards in batches
        for (let i = 0; i < allCardIds.length; i += 1000) {
          const batchCardIds = allCardIds.slice(i, i + 1000);
          const batchCards: any[] = await this.invokeAnki("cardsInfo", { cards: batchCardIds });
      ...
      const expressions: { expression: string; maxInterval: number }[] = [];
  ...
  async invokeAnki(action: string, params: Record<string, any> = {}): Promise<any> {
    try {
  ...
  pinyinTonesToNumbers(pinyin: string, charCount: number = 1): string {
    if (!pinyin) return '';
  ...
  async buildCardFields(wordData: any, fieldMappings: Record<string, string>): Promise<Record<string, string>> {
  ...
  async storeMediaFile(dataUrl: string, type: string): Promise<string | null> {
    try {
      if (!dataUrl) {
        return null;
      }
  ...
  extractBase64FromDataUrl(dataUrl: string): string | null {
  ...
  async findDuplicates(character: string, fieldMappings: Record<string, string>, noteType: string): Promise<any[]> {
    try {
  ...
  async handleSettingsChanged(settings: any, sendResponse: SendResponse): Promise<void> {
    try {
  ...
  async handleGetExtensionSettings(sendResponse: SendResponse): Promise<void> {
    try {
      await this.loadExtensionSettings();
      sendResponse({
        success: true,
   ...
   */
  async handleOnboardingCompleted(languageCode: string, sendResponse?: SendResponse): Promise<void> {
    try {
