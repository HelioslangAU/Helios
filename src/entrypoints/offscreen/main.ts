...
class DictionaryStorage {
   ...
   */
  async storeDictionary(languageCode: string, dictionary: Record<string, any>): Promise<void> {
    try {
   ...
   */
  async getDictionary(languageCode: string): Promise<Record<string, any> | null> {
    try {
      const db = await this.openDB();
   ...
   */
  async hasDictionary(languageCode: string): Promise<boolean> {
...
class OffscreenDictionaryService {
  ...
  async handleLoadDictionary(languageCode: string, nativeLanguageCode: string | null = null): Promise<any> {
    try {
   ...
   */
  async downloadAndProcessDictionary(languageCode: string, nativeLanguageCode: string | null = null): Promise<Record<string, any>> {
    try {
      ...
      let zipData: ArrayBuffer | null = await response.arrayBuffer();
      let zipBytes: Uint8Array | null = new Uint8Array(zipData);
      ...
      if (hasTermBanks) {
        ...
        const termBankFiles: { path: string; data: Uint8Array; number: number }[] = [];
  ...
  async handleGetDefinition(word: string): Promise<any> {
  ...
  async handleHasWord(word: string): Promise<any> {
    try {
      const adapter: any = this.languageRegistry.getAdapter();
  ...
  async handleGetEntries(words: string[]): Promise<any> {
    try {
      const adapter: any = this.languageRegistry.getAdapter();
  ...
  async handleGetSize(): Promise<any> {
    try {
      const size = Object.keys(this.dictionaryManager.dictionary).length;
