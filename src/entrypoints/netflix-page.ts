...
export default defineUnlistedScript(() => {
  ...
  const subTracks = new Map<any, Map<any, string>>(); // Store subtitle URLs by movieId -> trackId -> url
   ...
   */
  function extractUrlLegacy(track: any): string | undefined {
   ...
   */
  function extractUrl(track: any): string | undefined {
   ...
   */
  function storeSubTrack(video: any): void {
   ...
   */
  async function getNetflixSubtitles(): Promise<any[] | null> {
    try {
      const np = player();
   ...
   */
  async function loadSubtitleUrl(trackId: any, language: string): Promise<string | null> {
    try {
      const np = player();
      if (!np) {
        return null;
      ...
      // Find the track
      const track = np.getTimedTextTrackList()?.find((t: any) => t.trackId === trackId);
      if (!track) {
   ...
   */
  window.addEventListener('helios-netflix-request-subtitles', async (event: Event) => {
   ...
   */
  window.addEventListener('helios-netflix-load-subtitle-url', async (event: Event) => {
    const { trackId, language } = (event as CustomEvent).detail;
   ...
   */
  window.addEventListener('helios-netflix-request-subtitle-content', async (event: Event) => {
   ...
   */
  window.addEventListener('helios-netflix-seek-request', (event: Event) => {
